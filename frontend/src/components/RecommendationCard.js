...
import { Box, Card, CardActionArea, CardActions, CardHeader, Typography, Chip, IconButton, Tooltip } from "@mui/material"
...
export default function RecommendationCard(props) {
    ...
    const style = () => {
        return {
            card: {
                ...
                borderBottomColor: status === "implemented" ? "success.main" : status === "unapplicable" ? "error.main" : "primary.main",
            }
        }
    }
    ...
    return (
        <Card raised sx={style().card}>
            <CardActionArea sx={{height: "65%"}} onClick={handleClickRecommendation}>
                <CardHeader component={() =>
                    <>
                        <Typography align="left" sx={{padding: 1.5}} variant="body1">{headline}</Typography>
                    </>
                } />
            </CardActionArea>
            <CardActions>
